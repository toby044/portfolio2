<template>
  ...
  >
    <div v-for="item in projects" class="grid lg:grid-cols-2 lg:gap-x-24">
      <div
        class="c-project-text-container h-fit lg:sticky top-[var(--dynamic-site-header-height,64px)] pb-12 left-0"
      >
        <NuxtLink
          :to="item.url"
          target="_blank"
          external
          class="c-project-text-container-link--container flex items-center group"
        >
          <h3
            class="-ml-1 text-[3rem] sm:text-[5rem] md:text-[5.5rem] leading-[90%] lg:text-[4rem] xl:text-[5.5rem] lg:leading-[80%] lg:-tracking-[3.5px] font-bold block 2xl:w-[90%]"
          >
            <span v-html="item.title"></span>
            <Icon
              name="mdi:arrow-top-right"
              size="65px"
              class="h-[1lh] -mb-[13.5px] inline-block transition-transform duration-300 ease-[var(--easing-ease-slow)] group-hover:-translate-y-2.5 group-hover:translate-x-2.5"
            />
          </h3>
        </NuxtLink>
        <span
          class="max-w-[65ch] lg:max-w-[45ch] text-sm s-rich-text block mt-[1.5lh]"
          v-html="item.description"
        ></span>
      </div>
      <div v-if="item.images" class="min-h-screen flex flex-col gap-4">
        <NuxtImg
...
const projects = [
  {
    title: "Hovedstadens<br/> Beredskab",
    description:
